...
#include "usonic_sensor/tof_sensor.hpp"
...
TeraRangerSensor::TeraRangerSensor(uint8_t address, const char i2cDevice[]):
		I2CSensorBase(address)
{
...
void TeraRangerSensor::doMeasurement()
{
	...
	while (true) {
				try{
					tries++;
					bus->writeWord(rangeCMD);
					bus->readWordByte(rawMeas, &crc);
...
void TeraRangerSensor::restartI2CSensor()
{
	delete this->bus;
	this->initI2CSensor();
}
